                           ...
                           uint16_t x, uint16_t y, esp_painter_color_t color)
{
    ...
    if (using_ppa) {
        ESP_LOGD(TAG, "Drawing pixel at (%d,%d) with color 0x%04x", x, y, color_value);
        
        uint32_t offset = (y * painter->text_canvas.width + x) * 2; 
        if (offset + 2 > buffer_size) {
            ESP_LOGW(TAG, "Buffer overflow: offset %d, size %d", offset, buffer_size);
...
//             ESP_LOGE(TAG, "Buffer too small: need %u, have %u", aligned_size, buffer_size);
//             return ESP_ERR_INVALID_SIZE;
//         }
